#!/usr/bin/env python3
"""Normalize and validate PS1 texture packs for the EmuCoreR catalog.

The generated archive contains DuckStation ``vram-write-*`` replacement
textures only, stored at the archive root with any subfolders preserved.
The limits mirror TexturePackRepository and RemoteContentCatalogRepository
in the EmuCoreR Android application.
"""
...
TEXTURE_EXTENSIONS = {".png", ".jpg", ".tga", ".bmp"}
ROOT_MARKERS = {"textures", "replacements"}
...
def is_root_marker(part: str) -> bool:
...
def texture_relative_path(parts: tuple[str, ...], strip_components: int) -> tuple[str, ...]:
    ...
    root_index = next(
        (
            ...
            if is_serial(part) or lowered[index] in ROOT_MARKERS
        ),
        None,
    )
    if root_index is not None:
        start = root_index + 1
        while start < len(parts) and (is_serial(parts[start]) or lowered[start] in ROOT_MARKERS):
            start += 1
        relative = parts[start:]
    else:
        relative = parts

    if relative and CODELOAD_ROOT_RE.fullmatch(relative[0]):
        relative = relative[1:]
...
def has_tga_footer(stream: BinaryIO) -> bool:
    try:
        ...
        if size < 26:
            return False
        stream.seek(size - 26)
        return b"TRUEVISION-XFILE" in stream.read(26)
...
def validate_texture_header(stream: BinaryIO, extension: str, label: str) -> None:
    ...
    if extension == ".png":
        ...
        if len(header) < 3 or header[:3] != b"\xff\xd8\xff":
            raise PackError(f"invalid JPEG signature: {label}")
    elif extension == ".bmp":
        if len(header) < 2 or header[:2] != b"BM":
            raise PackError(f"invalid BMP signature: {label}")
    elif extension == ".tga":
        plausible_type = len(header) >= 3 and header[2] in TGA_IMAGE_TYPES
        if not plausible_type and not has_tga_footer(stream):
            raise PackError(f"invalid TGA image type or footer: {label}")
    else:
...
) -> tuple[list[tuple[SourceFile, str, str]], int]:
...
def prepare(input_path: Path, output_path: Path, strip_components: int) -> PackSummary:
    ...
    try:
        with source_files(input_path) as available_sources:
            sources, skipped_files = normalized_sources(available_sources, strip_components)
        ...
        if temp_path.stat().st_size > MAX_CATALOG_ARCHIVE_BYTES:
            raise PackError("normalized ZIP exceeds EmuCoreR's 16 GiB archive limit")
        shutil.move(str(temp_path), output_path)
        temp_path = None
        return validate(output_path, skipped_files)
    finally:
...
def inspect(path: Path, skipped_files: int = 0) -> tuple[PackSummary, dict[str, tuple[int, str]]]:
...
def validate(path: Path, skipped_files: int = 0) -> PackSummary:
    return inspect(path, skipped_files)[0]
