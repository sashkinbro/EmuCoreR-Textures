#!/usr/bin/env python3
"""Validate the EmuCoreR PS1 texture catalog (textures.json) and audit ledger."""
...
def main() -> None:
    ...
    if catalog_path.stat().st_size > MAX_CATALOG_BYTES:
        raise SystemExit("textures.json exceeds EmuCoreR's 8 MiB catalog limit")
    ...
    for index, entry in enumerate(entries):
        ...
        if isinstance(entry.get("sizeBytes"), int) and entry["sizeBytes"] > MAX_TEXTURE_ARCHIVE_BYTES:
            errors.append(f"{label}.sizeBytes exceeds EmuCoreR's 16 GiB limit")
        if isinstance(entry.get("fileCount"), int) and entry["fileCount"] > MAX_TEXTURE_FILE_COUNT:
            errors.append(f"{label}.fileCount exceeds EmuCoreR's 50,000-file limit")
